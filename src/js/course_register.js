App = {
  ...
  render: function() {
    ...
    App.contracts.program.deployed().then(function(instance) {
      ...
      for (var i = 1; i <= candidatesCount; i++) {
        electionInstance.getname(i).then(function(candidate) {
          var id=candidate[0];
          var name = candidate[1];
...
submit: function(event) {
  ...
  var pid=parseInt(document.getElementById('program').value);
...
App.contracts.student.deployed().then(function(instance) {
  ...
  return adoptionInstance.registerCourse(name,department,pid,sem, {from: account});
